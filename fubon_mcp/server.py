...
def on_order_changed(order_changed_data):
    ...
    """
    global latest_order_changed_reports
    try:
        # 添加時間戳到數據中
        timestamped_data = {
            'timestamp': datetime.now().isoformat(),
...
def modify_quantity(args: Dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            # 創建修改數量對象
            modify_obj = sdk.stock.make_modify_quantity_obj(target_order, new_quantity)
            
            # 執行修改數量
            result = sdk.stock.modify_quantity(account_obj, modify_obj)
            
            if result and hasattr(result, 'is_success') and result.is_success:
                return {
                    'status': 'success',
                    'data': result.data if hasattr(result, 'data') else result,
                    'message': f'成功修改委託 {order_no} 數量為 {new_quantity}'
                }
            else:
                return {
                    'status': 'error',
                    'data': None,
                    'message': f'修改委託 {order_no} 數量失敗'
                }
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'修改數量時發生錯誤: {str(modify_error)}'
            }
...
def get_account_info(args: Dict) -> dict:
    ...
    try:
        ...
        account_details = {
            'basic_info': {
                'name': getattr(target_account, 'name', 'N/A'),
                'branch_no': getattr(target_account, 'branch_no', 'N/A'),
                'account': getattr(target_account, 'account', 'N/A'),
                'account_type': getattr(target_account, 'account_type', 'N/A')
            }
        }
        
        try:
            # 獲取銀行水位
            bank_balance = sdk.accounting.bank_remain(target_account)
            if bank_balance and hasattr(bank_balance, 'is_success') and bank_balance.is_success:
                account_details['bank_balance'] = bank_balance.data
            else:
                account_details['bank_balance'] = None
        except Exception as bank_error:
            account_details['bank_balance'] = f'獲取銀行水位失敗: {str(bank_error)}'
        
        try:
            # 獲取未實現損益
            unrealized_pnl = sdk.accounting.unrealized_gains_and_loses(target_account)
            if unrealized_pnl and hasattr(unrealized_pnl, 'is_success') and unrealized_pnl.is_success:
                account_details['unrealized_pnl'] = unrealized_pnl.data
            else:
                account_details['unrealized_pnl'] = None
        except Exception as pnl_error:
            account_details['unrealized_pnl'] = f'獲取未實現損益失敗: {str(pnl_error)}'
        
        try:
            # 獲取交割資訊 (今日)
            settlement = sdk.accounting.query_settlement(target_account, "0d")
            if settlement and hasattr(settlement, 'is_success') and settlement.is_success:
                account_details['settlement_today'] = settlement.data
            else:
                account_details['settlement_today'] = None
        except Exception as settle_error:
            account_details['settlement_today'] = f'獲取交割資訊失敗: {str(settle_error)}'
    ...
    try:
        ...
        try:
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'獲取庫存資訊時發生錯誤: {str(inventory_error)}'
            }
    ...
    try:
        ...
        try:
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'獲取未實現損益時發生錯誤: {str(pnl_error)}'
            }
    ...
    try:
        ...
        try:
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'獲取交割資訊時發生錯誤: {str(settlement_error)}'
            }
    ...
    try:
        ...
        try:
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'獲取銀行水位時發生錯誤: {str(balance_error)}'
            }
    ...
    try:
        ...
        try:
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'獲取委託結果時發生錯誤: {str(order_error)}'
            }
...
def get_historical_stats(args: Dict) -> dict:
    ...
    try:
        ...
        return {
            'status': 'success',
            'data': result,
            'message': f'成功獲取 {symbol} 近 52 週數據'
        }
    except Exception as e:
        return {
            'status': 'error',
...
def get_all_reports(args: Dict) -> dict:
    ...
    try:
        ...
        global latest_order_reports, latest_order_changed_reports, latest_filled_reports, latest_event_reports
...
def modify_price(args: Dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            # 創建修改價格對象
            modify_obj = sdk.stock.make_modify_price_obj(target_order, str(new_price))
            
            # 執行修改價格
            result = sdk.stock.modify_price(account_obj, modify_obj)
            
            if result and hasattr(result, 'is_success') and result.is_success:
                return {
                    'status': 'success',
                    'data': result.data if hasattr(result, 'data') else result,
                    'message': f'成功修改委託 {order_no} 價格為 {new_price}'
                }
            else:
                return {
                    'status': 'error',
                    'data': None,
                    'message': f'修改委託 {order_no} 價格失敗'
                }
            ...
            return {
                'status': 'error',
                'data': None,
                'message': f'修改價格時發生錯誤: {str(modify_error)}'
            }
...
def batch_place_order(args: Dict) -> dict:
    ...
    try:
        ...
        # 使用 ThreadPoolExecutor 進行並行下單
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任務
            future_to_order = {
                executor.submit(place_single_order, order_data): order_data 
        ...
        return {
            ...
            'message': f'批量下單完成：總共 {len(orders)} 筆，成功 {len(successful_orders)} 筆，失敗 {len(failed_orders)} 筆'
        }
